...
{
    $pin_amount=$_POST[pins]*3600;
    $a_pin=$d_detail[pin_3_wallet]+$_POST[pins];
    $balaaa=$d_detail[main_wallet]-$pin_amount;
    $que="INSERT INTO `buy_pin_history` (`bph_id`, `d_id`, `date`, `time`, `pin_qty`, `amount`, `pin_gen_charge`, `tds_cut`, `total_amount`, `pin_type`) VALUES (NULL, '$_SESSION[d_id]', '$date', '$time', '$_POST[pins]', '$pin_amount', '0', '0', '$pin_amount', '3600');";
  ...
  //  $with_min=sub_withdrawal_amount($_SESSION[d_id], $pin_tot_amount, "Pin Purchased", '', 'admin');
    ...
    $up_que="UPDATE `distributor` SET `pin_3_wallet` = '$a_pin' WHERE `distributor`.`d_id` = $_SESSION[d_id];";
    
   
    
    $his_que="INSERT INTO `pin_wallet_3600` (`pw_id`, `d_id`, `date`, `time`, `pin`, `a_pin`, `note`, `type`) VALUES (NULL, '$_SESSION[d_id]', '$date', '$time', '$_POST[pins]', '$a_pin', 'Purchase From Admin', '+');";
    if($balaaa>=0){
    if($con->query($ins_mw)===TRUE && $con->query($u_mw)===TRUE && $con->query($que)===TRUE && $con->query($up_que)===TRUE && $con->query($his_que)===TRUE)
    {
		...
		///////////
        echo "<script>alert('Success! 3600Rs Pin Purchased Successfully '); location.href='pin_generate.php';</script>";
    }
    else{
        $ent_fail="INSERT INTO `entry_fail_report` (`efr_id`, `date`, `time`, `d_id`, `admin_id`, `url`, `failed_query_n`, `f_amt_id`, `app`) VALUES (NULL, '$date', '$time', '$_SESSION[d_id]', '', '$url', 'buy pin through withdrawal wallet', '', '');";
        $con->query($ent_fail);
