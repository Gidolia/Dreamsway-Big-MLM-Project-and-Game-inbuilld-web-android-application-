    ...
    $obj = json_decode($response);
        ...
        if($obj->TRNSTATUS == "1")
        {
           ...
           $savw="UPDATE `recharge_response` SET `hold_amount` = 'n' WHERE `recharge_response`.`unique_id` = '$_GET[ref_no]';";
    		...
    		echo "<script>location.href='mobile_recharge.php?status=s';</script>";
            
        }elseif($obj->TRNSTATUS == "0")
        {echo "<script>location.href='mobile_recharge.php?status=p';</script>";
        }
        ...
        echo "<script>location.href='mobile_recharge.php?status=f';</script>";
    }
?>