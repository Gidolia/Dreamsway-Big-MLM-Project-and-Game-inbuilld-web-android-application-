...
if(isset($_POST[confirm_activate]))
{
    ...
    {
    /////////////////// substracting PIN
    $pin=$d_detail[pin_2_wallet]-1;
    if($pin>=0)
    {
    $up_que="UPDATE `distributor` SET `pin_2_wallet` = '$pin' WHERE `distributor`.`d_id` = $_SESSION[d_id];";
...
$lsql="SELECT d_id,s_id,a_status FROM `distributor` WHERE `d_id`='$a_d_id'";
 ...
 if($fet[a_status]=="n")
 {
    ...
    $sql .="UPDATE `distributor` SET `activated_pin` = '2500' WHERE `distributor`.`d_id` = '$a_d_id';";
...
///////////////////////////level 1 500/-
 ...
 $fet1=mysqli_fetch_assoc($que1);
    ...
    $withdrawal_wallet=$fet1[withdrawal_wallet]+500;
    $sql .="INSERT INTO `withdrawal_wallet` (`ww_id`, `d_id`, `date`, `time`, `type`, `amount`, `a_bal`, `note`, `income_for`, `from_d_id`, `to_d_id`, `activated_id`, `level`) VALUES (NULL, '$fet1[d_id]', '$date', '$time', '+', '500', '$withdrawal_wallet', 'Level Income', 'level', '', '', '$a_d_id', '1');";
 ...
 $fet6=mysqli_fetch_assoc($que6);
    ...
    if ($con->multi_query($sql) === TRUE) {
        echo "<script>alert('Sucessfully Activated'); location.href='activate_id_pin2500.php';</script>";
    } else {
      echo "<script>alert('Failed! Plz Try Again'); location.href='activate_id_pin2500.php';</script>";
    }
 }
 else{echo "<script>alert('This Account is Already Activated'); location.href='activate_id_pin2500.php';</script>";}
}else{echo "<script>alert('You Dont have balance'); location.href='activate_id_pin2500.php';</script>";}
}else{
    echo "<script>alert('Sucessfully Activated'); location.href='activate_id_pin2500.php';</script>";
}
}
