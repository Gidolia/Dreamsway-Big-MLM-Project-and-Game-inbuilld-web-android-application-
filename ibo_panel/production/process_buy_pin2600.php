...
{
    $pin_amount=$_POST[pins]*2600;
    $a_pin=$d_detail[pin_2_wallet]+$_POST[pins];
    $balaaa=$d_detail[main_wallet]-$pin_amount;
    $que="INSERT INTO `buy_pin_history` (`bph_id`, `d_id`, `date`, `time`, `pin_qty`, `amount`, `pin_gen_charge`, `tds_cut`, `total_amount`, `pin_type`) VALUES (NULL, '$_SESSION[d_id]', '$date', '$time', '$_POST[pins]', '$pin_amount', '0', '0', '$pin_amount', '2500');";
  ...
  //  $with_min=sub_withdrawal_amount($_SESSION[d_id], $pin_tot_amount, "Pin Purchased", '', 'admin');
    ...
    $up_que="UPDATE `distributor` SET `pin_2_wallet` = '$a_pin' WHERE `distributor`.`d_id` = $_SESSION[d_id];";
   ...
   // $his_que="INSERT INTO `pin_wallet` (`pw_id`, `date`, `time`, `d_id`, `pin_qty`, `from_d_id`, `to_d_id`, `admin`, `type`, `note`, `req_no`) VALUES (NULL, '$date', '$time', '$_SESSION[d_id]', '$_POST[pins]', 'admin', '', 'y', '+', 'Purchase From Admin', '');";
    ...
    $his_que="INSERT INTO `pin_wallet_2600` (`pw_id`, `d_id`, `date`, `time`, `pin`, `a_pin`, `note`, `type`) VALUES (NULL, '$_SESSION[d_id]', '$date', '$time', '$_POST[pins]', '$a_pin', 'Purchase From Admin', '+');";
    if($balaaa>=0){
    if($con->query($ins_mw)===TRUE && $con->query($u_mw)===TRUE && $con->query($que)===TRUE && $con->query($up_que)===TRUE && $con->query($his_que)===TRUE)
    {
		...
		///////////
        echo "<script>alert('Success! 2500Rs Pin Purchased Successfully '); location.href='pin_generate.php';</script>";
    }
    else{
