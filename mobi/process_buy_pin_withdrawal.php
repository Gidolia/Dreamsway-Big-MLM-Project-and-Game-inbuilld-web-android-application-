...
if(isset($_POST[confirm_buy_pin]))
{
  ...
  //  $with_min=sub_withdrawal_amount($_SESSION[d_id], $pin_tot_amount, "Pin Purchased", '', 'admin');
    ...
    if($con->query($ins_mw)===TRUE && $con->query($u_mw)===TRUE && $con->query($que)===TRUE && $con->query($up_que)===TRUE && $con->query($his_que)===TRUE)
    {
		...
		///////////
        echo "<script>location.href='generate_pin_withdrawal_wallet.php?s=s';</script>";
    }
    else{
        ...
        $con->query($ent_fail);
        echo "<script>location.href='generate_pin_withdrawal_wallet.php?s=q_f';</script>";
    }
    }else{echo "<script>location.href='generate_pin_withdrawal_wallet.php?s=u_bal';</script>";
    }
