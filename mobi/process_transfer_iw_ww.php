    ...
    if($_POST[amount]>=500){
        ...
        if($a_income_wallet_bal>=0)
        {
            ...
            if($con->multi_query($u_iw)===TRUE){
                echo "<script>location.href='transfer_to_withdrawal_wallet.php?s=s';</script>";
            }
            else{
                $erf="INSERT INTO `entry_fail_report` (`efr_id`, `date`, `time`, `d_id`, `admin_id`, `url`, `failed_query_n`, `f_amt_id`, `app`) VALUES (NULL, '$date', '$time', '$_SESSION[d_id]', '', '$url', 'wallet transfering', '', 'y');";
                echo "<script>location.href='transfer_to_withdrawal_wallet.php?s=q_f';</script>";
            }
        }else{
             echo "<script>location.href='transfer_to_withdrawal_wallet.php?s=u_bal';</script>";
        }
        }
        else{
             echo "<script>location.href='transfer_to_withdrawal_wallet.php?s=n_id';</script>";
        }
    }
    else{echo "<script>location.href='transfer_to_withdrawal_wallet.php?s=amt';</script>";}
}
