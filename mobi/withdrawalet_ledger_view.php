            ...
            <div class="main-content-inner">
             <div class="row">
                ...
                <div class="col-12 mt-12">
                    <div class="card">
                        <div class="card-body">
                            <h4>Withdrawal Wallet Ledger View</h4>
                            <br>&nbsp;
                            
                          <?php
        
                             
        $g="SELECT * FROM `main_wallet` WHERE `d_id`='$_SESSION[d_id]' ORDER BY `main_wallet`.`mw_id` DESC";
        $dc=$con->query($g);
        ?>
        ...
        <table class="table text-center" id="dataTable">
            ...
            <tr>
                ...
                <td><?php  if($d[type]=="-"){ ?> <span class="badge badge-danger">Debited</span> <?php }
                          ...
                          ?></td>
                <td><?php 
                if($d[note]=="transfered")
                {
                    
                    if($d[from_d_id]!="")
                    {
                        $sqlccc="SELECT * FROM `distributor` WHERE `d_id`='$d[from_d_id]'";
                        $sq=$con->query($sqlccc);
                        $xx=$sq->fetch_assoc();
                        echo "Received ".$xx[name]."(DS".$d[from_d_id].")";
                    }
                    elseif($d[to_d_id]!="")
                    {
                        $sqlccc="SELECT * FROM `distributor` WHERE `d_id`='$d[to_d_id]'";
                        $sq=$con->query($sqlccc);
                        $xx=$sq->fetch_assoc();
                        echo "Transfered ".$xx[name]."(DS".$d[to_d_id].")";
                    }
                }
                else{
                echo $d[note];}?></td>
